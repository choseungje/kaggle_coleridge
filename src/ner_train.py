...
# train2.to_csv('../input/train_ner_only_pos.csv', index=False)
# print("Save")

# ==========================================================================================
test=pd.read_csv('../input/train_ner_only_pos.csv')
...
train_x, valid_x, train_y, valid_y = train_test_split(
    inputs, target, test_size=0.1, shuffle=True, random_state=45)
...
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=4100 * 15, eta_min=5 * 1e-7)


def save_torch(model, optimizer):
    if not os.path.exists('../output/'):
        os.makedirs('../output/')
    torch.save(model, '../output/ner_model.pt')
    state = {
        'State_dict': model.state_dict(),
        'optimizer': optimizer.state_dict()
    }
    torch.save(state, '../output/ner_model_state.pt')
